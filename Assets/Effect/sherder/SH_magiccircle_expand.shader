Shader "Unlit/SH_magiccircle_expand"
{
    Properties
    {
        ...
        _StartScale ("Start Scale", Range(0.01, 3)) = 0.1
        _EndScale ("End Scale", Range(0.01, 5)) = 1.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 centerUV = i.uv - center;

                float progress = saturate(_Progress);
                float scale = lerp(_StartScale, _EndScale, progress);

                float2 expandUV = centerUV / max(scale, 0.0001) + center;
                ...
                // 3. テクスチャ取得
                //-----------------------------------
                fixed4 texColor = tex2D(_MainTex, expandUV);
                ...
                // 4. 広がるほど消えるフェード
                //-----------------------------------
                float fade = pow(saturate(1.0 - progress), _FadePower);
