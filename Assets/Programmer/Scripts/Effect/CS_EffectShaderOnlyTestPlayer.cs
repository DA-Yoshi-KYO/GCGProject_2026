...
public class CS_EffectShaderOnlyTestPlayer : MonoBehaviour
{
    ...
    [SerializeField]
    private Vector3 v3_TestMoveOffset = new Vector3(0.0f, 1.0f, 0.0f);
    [SerializeField]
    private Vector3 v3_TestRotateEulerAngles = new Vector3(0.0f, 180.0f, 0.0f);
    [SerializeField]
    private Vector3 v3_TestScale = new Vector3(2.0f, 2.0f, 2.0f);
    [SerializeField]
    ...
    private void PlayTestEffect()
    {
        ...
        {
            csad_CurrentTestEffect = cs_EffectPlayer.PlayEffect(
                csst_EffectPlayData,
        ...
        csad_CurrentTestEffect = cs_EffectPlayer.PlayEffect(csst_EffectPlayData);
    }
    ...
    private void MoveCurrentEffectTest()
    {
        if (TryGetCurrentEffectTransformController(
            out CS_EffectTransformController cs_EffectTransformController) == false)
        {
            return;
        }

        Vector3 v3_TargetPosition =
            csad_CurrentTestEffect.transform.position + v3_TestMoveOffset;
        ...
        out CS_EffectTransformController cs_EffectTransformController)
    {
        ...
        if (csad_CurrentTestEffect == null)
        {
            Debug.LogWarning("[CS_EffectShaderOnlyTestPlayer] 現在再生中のEffectがありません。");
            return false;
        }

        cs_EffectTransformController =
            csad_CurrentTestEffect.GetComponent<CS_EffectTransformController>();

        if (cs_EffectTransformController == null)
        {
            Debug.LogWarning("[CS_EffectShaderOnlyTestPlayer] CS_EffectTransformControllerがEffectに付いていません : " + csad_CurrentTestEffect.name);
            return false;
        }
