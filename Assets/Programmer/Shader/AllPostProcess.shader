Shader "Custom/AllPostProcess"
{
    ...
    SubShader
    {
        ...
        /*
            Mainの色を出力する関数
            ここを起点にポストプロセスを構築していく
        */

        SurfaceDescription SurfaceDescriptionFunction(SurfaceDescriptionInputs IN)
        {
            float4 mainTex = Unity_Universal_SampleBuffer_BlitSource_float(float4(IN.NDCPosition.xy, 0, 0).xy); // 画面の色
            SurfaceDescription surface = (SurfaceDescription)0;
            surface.BaseColor = mainTex.rgb;
            surface.Alpha = float(1);

            // // 例：グレースケール化
            if (_Used)
            {
                float gray = dot(surface.BaseColor, float3(0.299, 0.587, 0.114)); // NTSCの輝度計算式
                surface.BaseColor = float3(gray, gray, gray);
            }
            // // 例：セピア調化
            // float3 sepiaColor = float3(1.2, 1.0, 0.8); // セピア調の色
            // surface.BaseColor *= sepiaColor;
